...
function Chat({ pp, contact, msg, time, unreadMsgs, active, chatId, handleClick }) {
  return (
    // Chat container
    <div
      onClick={() => {
    ...
    >
      {/* Profile picture */}
      <Image src={pp} alt="profile_picture" className="rounded-full w-[50px] mr-5" />
      ...
      <div className="flex justify-between border-t border-[#e9edef] w-full h-full py-3">
        ...
        <div className="flex flex-col justify-between text-[#111b21]">
          ...
          <h1
            className={`mb-1 ${
              !unreadMsgs ? "text-[#111b21] font-medium" : "text-[#121e25] font-semibold"
            }`}
          >
            {contact}
          </h1>
